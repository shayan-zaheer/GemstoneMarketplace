...
const Login = () => {
  ...
  return (
    <div className="w-full min-h-[94vh] flex justify-center items-center bg-[#1a1c1ff8] mt-16">
      <div
        onClick={handleClick}
        className={`relative max-sm:w-[350px] sm:w-96 cursor-pointer transition-all duration-500 ease-linear bg-[#2A2D33] p-6 rounded-xl overflow-hidden ${isClicked ? "h-[340px]" : "h-32"}`}
      >
        ...
        <div
          className={`relative z-10 transition-opacity duration-500 ${isClicked ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        >
          <LoginForm />
        </div>

        <div
          className={`relative z-10 flex justify-between text-white transition-opacity duration-500 my-3 ${isClicked ? "opacity-100" : "opacity-0 pointer-events-none hidden"}`}
        >
